...
// ============================================================
export function generateDataset(seed, profile = VANTIX_PROFILE) {
  ...
  const revAtRiskK = round(between(720, 950), 1);
  ...
  const retentionPlan = profile.retentionPlan.map((r) => {
    ...
    const clv = round(jitter(r.baseClv, 0.12));
    const impact = Math.min(clv, round(jitter(r.baseImpact, 0.12)));
  ...
  const kpis = [
    { label: "Revenue at Risk",        val: `$${revAtRiskK.toFixed(1)}K`, d: `-${revAtRiskDelta}%`, good: false, spark: spark(revAtRiskK * 0.048, revAtRiskK * 0.034, 8, 0) },
  ...
  const revenueAtRisk = { totalK: revAtRiskK, accounts: atRiskAccounts, recoverK };
  ...
  return {
    seed,
    metrics: { nrr, nrrDelta, clvCac, churn, revAtRiskK, atRiskAccounts, recoverK, cohortGap },
    kpis,
    revenueAtRisk,
    retentionPlan,
    channels,
